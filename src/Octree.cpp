...
// gets verrtices from the mesh annd..... builds a bounding box from the mesh
//
...
// We''l use this
// pointsRtn is probably the one we'll use because it is modified
// 
// something ... which box is in each child points????
//
...
//      
//tips
//make sure it works with 2 levels first then it can work with n levels
             
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	if (level >= numLevels) return;
	level++;
	// subdvide algorithm implemented here
	vector<Box> boxChildren;
	subDivideBox8(node.box, boxChildren); //tboxList has new value of 8

	for (int i = 0; i < boxChildren.size(); i++) {
		TreeNode child;
		child.box = boxChildren[i];
	...
	for (int j = 0; j < node.children.size(); j++) {
		if (node.children[j].points.size() > 1) { //children size might be differnt?
			subdivide(mesh, node.children[j], numLevels, level);
		}
	}
...
//
//readme file show example on how to drawo an octree, code there, but draw each level in a different color by creating
//an array of 12 colors and use index and draw color before u draw
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	if (node.points.size() == 1) {
		nodeRtn = node; //selected node 
		return true;
	}

	if (node.box.intersect(ray, 0.1, 1000)) { //if it is a hit go a level down
...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn) {
	if (node.points.size() == 1) {
		...
		//cout << endl << "box intersect working";
		return true;
	}
...
//we start at numLevel/level = 1 which is just the bounding box
// level 2 is 8 boxes, then next level = 3 has 8 boxes within each 8 boxes, and so on
//for every level we draw the octree children until we stop at numLevels(which is the slider)
...
// Optional
// can be handy for collision, just recurse down... something one point
