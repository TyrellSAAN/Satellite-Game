 ...
 //     assert(min < max);
	...
	//
	 bool overlap(const Box &box) {
		 ...
		 if (parameters[1].x() > box.parameters[0].x() && parameters[0].x() < box.parameters[1].x()) {
			 x = true;
		}
		 if (parameters[1].y() > box.parameters[0].y() && parameters[0].y() < box.parameters[1].y()) {
			 y = true;
		 }
		 if (parameters[1].z() > box.parameters[0].z() && parameters[0].z() < box.parameters[1].z()) {
			 z = true;
		 }
		 if (x && y && z) {
			 return true;
